 ...
 * @author Hairian
 */
@Named("brgAutoComplete")
@ConversationScoped
@Getter
@Setter
public class BrgAutoComplete implements java.io.Serializable {
    ...
    List<String> idbarang = new ArrayList<>();
    ...
    public List<Barang> completeBarangContains(String query) {
        if (page.getValueBegawi("decanting").equals('M')) {
            allBarang = referensi.onLoadListMappingArray(idbarang);
        } else {
            allBarang = referensi.onLoadList();
        }
        List<Barang> filteredBarangs = new ArrayList<>();
