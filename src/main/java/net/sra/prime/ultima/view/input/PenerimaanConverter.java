...
@Getter
@Setter
public class PenerimaanConverter implements java.io.Serializable, Converter {
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                Penerimaan penerimaan = servicePenerimaan.onLoad(value);
                return penerimaan;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((Penerimaan) object).getNo_penerimaan());
        } else {
            return null;
        }
    }
