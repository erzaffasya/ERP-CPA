...
@Getter
@Setter
public class PenawaranConverter implements java.io.Serializable, Converter {
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                Penawaran penawaran = servicePenawaran.selectOnePenawaran(value);
                return penawaran;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((Penawaran) object).getNomor());
        } else {
            return null;
        }
    }
