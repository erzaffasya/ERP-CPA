...
@Getter
@Setter
public class ArConverter implements java.io.Serializable, Converter {
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                AccArFaktur accArFaktur = referensi.selectOneAccArFaktur(value);
                return accArFaktur;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((AccArFaktur) object).getAr_number());
        } else {
            return null;
        }
    }
