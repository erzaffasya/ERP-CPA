...
@Getter
@Setter
public class JabatanConverter implements java.io.Serializable, Converter {

    @Autowired
    ServiceMasterJabatan serviceMasterJabatan;        
    
    @Override
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                MasterJabatan masterJabatan = serviceMasterJabatan.onLoad(Integer.parseInt(value));
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((MasterJabatan) object).getId_jabatan());
        } else {
            return null;
        }
    }
