...
@Getter
@Setter
public class CustomerConverter implements java.io.Serializable, Converter {
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                Customer customer = serviceCustomer.selectOne(value);
                return customer;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((Customer) object).getId_kontak());
        } else {
            return null;
        }
    }
