...
@Getter
@Setter
public class PackinglistConverter implements java.io.Serializable, Converter {
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                Packinglist packinglist = servicePackinglist.onLoad(value);
                return packinglist;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((Packinglist) object).getNomor());
        } else {
            return null;
        }
    }
