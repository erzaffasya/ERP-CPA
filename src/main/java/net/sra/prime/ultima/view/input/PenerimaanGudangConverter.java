...
@Getter
@Setter
public class PenerimaanGudangConverter implements java.io.Serializable, Converter {
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                return servicePenerimaanGudang.onLoad(value);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null) {
            return String.valueOf(((PenerimaanGudang) object).getNo_penerimaan());
        } else {
            return null;
        }
    }
