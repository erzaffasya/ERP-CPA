...
@Setter
@Getter
public class ControllerBarangConsumable {
     private static final long serialVersionUID = 8811960521862002964L;

    @Autowired
    ServiceBarangConsumable serviceBarangConsumable;
    
    private BarangConsumable item;
    private List<BarangConsumable> lBarangConsumable = new ArrayList<>();

    @PostConstruct
    public void init() {
        item = new BarangConsumable();
    ...
    public void onLoadList() {
        try {
            lBarangConsumable = serviceBarangConsumable.onLoadList();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }

    public List<BarangConsumable> getDataBarangConsumable() {
        return lBarangConsumable;
    }
    ...
    public void delete(Integer id) {
        ...
        try {
            serviceBarangConsumable.delete(id);
    ...
    public void tambah() {
        ...
        context.getExternalContext().getFlash().setKeepMessages(true);
        try {
            serviceBarangConsumable.tambah(item);
    ...
    public void onLoad() {
        item = serviceBarangConsumable.onLoad(item.getId());
    }
    ...
    public void ubah() {
        ...
        context.getExternalContext().getFlash().setKeepMessages(true);
        try {
            serviceBarangConsumable.ubah(item);
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Informasi", "Data Berhasil Diinput"));
            context.getExternalContext().redirect("./list.jsf");
        } catch (Exception e) {
            e.printStackTrace();
