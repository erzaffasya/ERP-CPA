...
import net.sra.prime.ultima.service.ServicePemakaian;
...
@Setter
@Getter
public class ControllerPemakaian implements java.io.Serializable {

    private static final long serialVersionUID = -1767732220818787987L;
    private List<Pemakaian> lPemakaian = new ArrayList<>();
    private Pemakaian item;
    private List<PemakaianDetail> lPemakaianDetail = new ArrayList<>();
    ...
    @Autowired
    ServicePemakaian servicePemakaian;
    ...
    public void initItem() {
        item = new Pemakaian();
        lPemakaianDetail = new ArrayList<>();
        lPemakaianDetail.add(new PemakaianDetail());
        status = 'D';
    ...
    public void nomorurut() {
        ...
        String noMax = servicePemakaian.noMax(item.getId_gudang(), Integer.parseInt(bulan), Integer.parseInt(year));
        String inisial = servicePemakaian.selectOneGudang(item.getId_gudang()).getInisial();
        if (noMax == null) {
            item.setNomor("001/OUT/" + inisial + "/" + bulan + tahun);
        } else {
            Integer nomor = Integer.parseInt(noMax);
            nomor = nomor + 1;
            noMax = String.format("%03d", nomor);
            item.setNomor(noMax + "/OUT/" + inisial + "/" + bulan + tahun);
        }
    }
    ...
    public void onLoadList() {
        ...
        try {
            if ((awal != null && akhir != null) || (awal == null && akhir == null)) {
                lPemakaian = servicePemakaian.onLoadList(awal, akhir, page.getMyPegawai().getId_pegawai(), status);
            } else {
    ...
    public void onLoadListJurnal() {
        ...
        try {
            if ((awal != null && akhir != null) || (awal == null && akhir == null)) {
                lPemakaian = servicePemakaian.onLoadListJurnal(awal, akhir, status);
            } else {
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Periode Salah !!!!", ""));
            }
    ...
    public List<Pemakaian> getDataPemakaian() {
        return lPemakaian;
    }
    ...
    public List<PemakaianDetail> getDataPemakaianDetail() {
        return lPemakaianDetail;
    ...
    public void onDeleteClicked(PemakaianDetail pemakaianDetail) {
        lPemakaianDetail.remove(pemakaianDetail);
    ...
    public void updateMaintenance() throws IOException {
        FacesContext context = FacesContext.getCurrentInstance();
        context.getExternalContext().redirect("./pemakaian-edit.jsf?id=" + item.getId());
    }
    ...
    public void onLoad() {
        options.setIdpegawai(page.getMyPegawai().getId_pegawai());
        try {
            item = servicePemakaian.onLoad(item.getId());
            lPemakaianDetail = servicePemakaian.onLoadDetail(item.getId());
    ...
    public void onLoadJurnal() {
        ...
        try {
            item = servicePemakaian.onLoad(item.getId());
            lPemakaianDetail = servicePemakaian.onLoadDetail(item.getId());
            lAccGlDetail = servicePemakaian.jurnalAwal(item);
            this.hitungSelisih();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public void tambah() {
        ...
        try {
            ...
            servicePemakaian.tambah(item, lPemakaianDetail);
    ...
    public void onBarangSelect(PemakaianDetail s, Integer i) {
    ...
    public void ubah(Character status) {
        ...
        if (status.equals('A')) {
            for (int i = 0; i < lPemakaianDetail.size(); i++) {
                if (lPemakaianDetail.get(i).getQty() == 0 || lPemakaianDetail.get(i).getQty() == null) {
                    context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Qty  tidak boleh kosong !!!!", ""));
                    benar = false;
                }
            }
        }
        if (benar) {
            try {
                ...
                servicePemakaian.ubah(item, lPemakaianDetail);
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Informasi", "Data Berhasil Diinput"));
            } catch (Exception e) {
                item.setStatus(st);
    ...
    public void maintenance() {
        FacesContext context = FacesContext.getCurrentInstance();
        try {
            servicePemakaian.maintenance(item);
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Informasi", "Data Berhasil Diinput"));
        } catch (Exception e) {
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, e.getMessage(), ""));
        }
    ...
    public void posting(Character status) {
        ...
        if (benar && jmlDebit.equals(jmlKredit)) {
            ...
            try {
                item.setStatus(status);
                servicePemakaian.posting(item, lPemakaianDetail, lAccGlDetail);
    ...
    public void nomorurutCancel() {
        ...
        String noMax = servicePemakaian.noMaxCancel(Integer.parseInt(bulannya), Integer.parseInt(tahun));

        if (noMax == null) {
            item.setNo_cancel("001/OUT-CANCEL/" + bulan + "/" + tahun);
        } else {
            Integer nomor = Integer.parseInt(noMax);
            nomor = nomor + 1;
            noMax = String.format("%03d", nomor);
            item.setNo_cancel(noMax + "/OUT-CANCEL/" + bulan + "/" + tahun);
        }
    }

    public void cancelPemakaian() {
        FacesContext context = FacesContext.getCurrentInstance();
        nomorurutCancel();
        try {
            item.setStatus('C');
            servicePemakaian.cancelPemakaian(item, lPemakaianDetail, page.getMyPegawai().getId_pegawai());
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Informasi", "Data Berhasil Diinput"));
        } catch (Exception e) {
            e.printStackTrace();
            item.setStatus('A');
    ...
    public void onTglPemakaian() {
        FacesContext context = FacesContext.getCurrentInstance();
        context.getExternalContext().getFlash().setKeepMessages(true);
        DateFormat bln = new SimpleDateFormat("MM");
        if (item.getTanggal().after(new Date())) {
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Tgl pemakaian tidak boleh lebih besar daripada tanggal hari ini !!!", ""));
            item.setTanggal(null);
        } else if (item.getStatus() != null) {
            Pemakaian pemakaian = servicePemakaian.onLoad(item.getId());
            if (!bln.format(item.getTanggal()).equals(bln.format(pemakaian.getTanggal()))) {
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Tgl pemakaian tidak boleh beda bulannya !!!", ""));
                item.setTanggal(pemakaian.getTanggal());
            }
        }else{
    ...
    public void onTglPemakaianSelect(SelectEvent event) {
        onTglPemakaian();
    }

    public void onTglPemakaianChange(AjaxBehaviorEvent event) {
        onTglPemakaian();
    }
    ...
    public void onLoadListReport() {
        ...
        try {
            if (id_barang != null) {
                if ((awal != null && akhir != null) || (awal == null && akhir == null)) {
                    lPemakaianDetail = servicePemakaian.onLoadReport(id_barang, awal, akhir, gudang, status);
                } else {
                    context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Periode Salah !!!!", ""));
                }
        ...
        try {
            if (id_barang != null) {
                if ((awal != null && akhir != null) || (awal == null && akhir == null)) {
                    ...
                    XSSFSheet sheet = workbook.createSheet("Laporan Pemakaian Barang");
                    ...
                    cell.setCellValue("Laporan Pemakaian Barang");
                    cell.setCellStyle(cellStyle);
                    ...
                    for (int i = 0; i < lPemakaianDetail.size(); i++) {
                        ...
                        cell = row.createCell(1);
                        cell.setCellValue(lPemakaianDetail.get(i).getNomor());
                        cell = row.createCell(2);
                        cell.setCellValue(lPemakaianDetail.get(i).getTanggal());
                        cell.setCellStyle(dateCellStyle);
                        cell = row.createCell(3);
                        cell.setCellValue(lPemakaianDetail.get(i).getKeterangan());
                        cell.setCellValue(Jsoup.parse(lPemakaianDetail.get(i).getKeterangan()).text());
                        cell = row.createCell(4);
                        cell.setCellValue(lPemakaianDetail.get(i).getQty());
                        cell.setCellStyle(style);
                        cell = row.createCell(5);
                        cell.setCellValue(lPemakaianDetail.get(i).getSatuan_kecil());
                        cell = row.createCell(6);
                        cell.setCellValue(lPemakaianDetail.get(i).getNote());
                        cell = row.createCell(7);
                        if (lPemakaianDetail.get(i).getStatus().equals('A')) {
                            cell.setCellValue("Approve");
                        } else if (lPemakaianDetail.get(i).getStatus().equals('P')) {
                            cell.setCellValue("Posting");
                        }
                        if (lPemakaianDetail.get(i).getStatus().equals('C')) {
                    ...
                    externalContext.setResponseHeader("Content-Disposition", "attachment; filename=\"Laporan Pemakaian Barang.xlsx\"");
