...
public interface MapperPenambahanCancel {

    @Insert("INSERT INTO penambahan_cancel (nomor,no_penambahan,tanggal,pesan,kode_user) "
            + "VALUES (#{nomor},#{no_penambahan},LOCALTIMESTAMP,#{pesan},#{kode_user})")
    public int insert(PenambahanCancel penambahanCancel) throws RuntimeException;
    
            
    @Select("SELECT MAX(SUBSTR(a.nomor,1,3)) "
            + " FROM penambahan_cancel a "
