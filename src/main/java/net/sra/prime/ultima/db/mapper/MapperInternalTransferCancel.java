...
public interface MapperInternalTransferCancel {

    @Insert("INSERT INTO internaltransfer_cancel (nomor,no_it,tanggal,pesan,kode_user) "
            + "VALUES (#{nomor},#{no_it},LOCALTIMESTAMP,#{pesan},#{kode_user})")
    public int insert(InternalTranferCancel internalTranferCancel) throws RuntimeException;
    
            
    @Select("SELECT MAX(SUBSTR(a.nomor,1,3)) "
            + " FROM internaltransfer_cancel a "
