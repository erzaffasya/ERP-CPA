...
@Setter
@Getter
public class ServiceResign {

    @Autowired
    MapperResign referensi;
    
    @Autowired
    MapperHrJabatan mapperHrJabatan;

    @Transactional(readOnly = true)
    public List<Resign> onLoadList() {
    ...
    @Transactional(readOnly = false)
    public void tambah(Resign item) {
        referensi.insert(item);
    }
    
    
    @Transactional(readOnly = false)
    public void ubah(Resign item) {
        referensi.update(item);
    }

    @Transactional(readOnly = true)
    public Resign onLoad(Integer id) {
