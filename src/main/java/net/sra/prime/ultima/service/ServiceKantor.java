...
@Setter
@Getter
public class ServiceKantor {

    @Autowired
    MapperKantor referensi;
    
    @Autowired
    MapperAccount mapperAccount;

    @Transactional(readOnly = true)
    public List<InternalKantorCabang> onLoadList() {
    ...
    @Transactional(readOnly = false)
    public void tambah(InternalKantorCabang item) {
        referensi.insert(item);
    }

    @Transactional(readOnly = false)
    public void ubah(InternalKantorCabang item) {
        referensi.update(item);
    }

    @Transactional(readOnly = true)
    public InternalKantorCabang onLoad(String id) {
        return referensi.selectOne(id);
    }
    
    @Transactional(readOnly = true)
    public InternalKantorCabang selectOne(String id) {
        return referensi.selectOne(id);
    }

    @Transactional(readOnly = true)
    public Account selectOneAccount(Integer id) {
        return mapperAccount.selectOne(id);
    }
    
    @Transactional(readOnly = true)
    public List<Region> selectRegion() {
        return referensi.selectRegion();
    }
