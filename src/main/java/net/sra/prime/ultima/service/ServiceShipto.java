...
@Setter
@Getter
public class ServiceShipto {
    
    @Autowired
    MapperShipto referensi;

    
    @Transactional(readOnly = true)
    public List<Shipto> onLoadList() {
        return referensi.select();
    }
    ...
    @Transactional(readOnly = false)
    public void tambah(Shipto item) {
        referensi.insert(item);
    }

    @Transactional(readOnly = false)
    public void ubah(Shipto item) {
        referensi.update(item);
    }

    @Transactional(readOnly = true)
    public Shipto onLoad(String id) {
